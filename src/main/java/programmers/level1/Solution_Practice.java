...
public class Solution_Practice {
     ...
     * @param num
     * @return 몇 번이나 반복해야 하는지 반환
     * */
    public int solution24(long num) {
        int answer = 0;
        if(num == 1) return 0;
     ...
     * @param arr : arr은 길이 1 이상, 100 이하인 배열입니다.
     * @return 정수를 담고 있는 배열 arr의 평균값을 return
     * */
    public double solution25(int[] arr) {
     ...
     * @param x : 1 이상, 10000 이하인 정수입니다.
     * @return 자연수 x를 입력받아 x가 하샤드 수인지 아닌지 검사하는 함수
     * */
    public boolean solution26(int x) {
        int n = x;
        int sum = 0;
        while(n > 0){
            sum += n % 10;
